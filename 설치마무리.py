...
# ※ 짝: 점검.py 에 같은 이름의 함수가 있다(점검.py 는 홀로 도는 파일이라 딸려 갈 수 없다).
#    고칠 때 둘을 함께 고친다.
# ── 프로그램 찾기 ───────────────────────────────────────────
...
def 새터미널창(파일이름, 안내, 명령, 확인클로드=""):
    ...
    if 윈도우:
        ...
        줄들 += ["Write-Host ''",
                 ...
                 "Write-Host ''"]
        if 확인클로드:
            # 파워셸 변수 이름은 ASCII 로만. 한국어 윈도우에서 한글 이름은 파서를 죽인다.
            줄들 += [f"$got = (& {홑(확인클로드)} auth status 2>$null | Out-String)",
                     "if ($got -match '\"loggedIn\"\\s*:\\s*true')"
                     " { Write-Host '  ✓ 로그인됐습니다.' }"
    ...
    for 창, 앞 in (("x-terminal-emulator", ["-e"]), ("gnome-terminal", ["--"]),
        ...
        어디 = 찾기(창)          # 있기만 한 것 말고 **도는 것**으로 고른다
        if not 어디:
            continue
        try:
